...
namespace ya
{
	float Character04::mCurHp = 100;
	float Character04::mMaxHp = 100;
	float Character04::mHpPercent = (Character04::mCurHp / Character04::mMaxHp) * 100;

	bool Character04::mBig = false;
	bool Character04::mSml = false;
	...
	void Character04::Initialize()
	{
		Transform* tr = GetComponent<Transform>();
		tr->SetScale(Vector2(1.2f, 1.2f));

		Image* mImage = Resources::Load<Image>(L"Char04", L"..\\Resources\\Char\\B\\cream.bmp");
		mAnimator = AddComponent<Animator>();
		mAnimator->CreateAnimation(L"idle", mImage, Vector2((346.33f * 1), (346.33f * 2)), 15, 6, 4, Vector2::Zero, 0.13, 0);
		mAnimator->CreateAnimation(L"Roll", mImage, Vector2::Zero, 15, 6, 6, Vector2::Zero, 0.15, 0);
		mAnimator->CreateAnimation(L"BeforeRun", mImage, Vector2((346.33f * 6), (346.33f * 0)), 15, 6, 1, Vector2::Zero, 0.1, 0);
		mAnimator->CreateAnimation(L"Run", mImage, Vector2(0.0f, (346.33f * 1)), 15, 6, 4, Vector2::Zero, 0.1, 0);
		mAnimator->CreateAnimation(L"FastRun", mImage, Vector2((346.33f * 4), (346.33f * 1)), 15, 6, 4, Vector2::Zero, 0.1, 0);
		mAnimator->CreateAnimation(L"SlowRun", mImage, Vector2(0.0f, (346.33f * 1)), 15, 6, 4, Vector2::Zero, 0.5, 0);
		mAnimator->CreateAnimation(L"Jump", mImage, Vector2((346.33f * 0), (346.33f * 0)), 15, 6, 1, Vector2::Zero, 0.1, 0);// 0�� �������� �ִϸ��̼� ���� ����
		mAnimator->CreateAnimation(L"DJump", mImage, Vector2((346.33f * 1), (346.33f * 0)), 15, 6, 6, Vector2::Zero, 0.08, 0);
		mAnimator->CreateAnimation(L"Slide", mImage, Vector2((346.33f * 9), (346.33f * 0)), 15, 6, 2, Vector2::Zero, 0.08, 0);
		mAnimator->CreateAnimation(L"Death", mImage, Vector2((346.33f * 6), (346.33f * 4)), 15, 6, 4, Vector2::Zero, 0.15, 0);
		mAnimator->CreateAnimation(L"Transp1", mImage, Vector2((346.33f * 9), (346.33f * 1)), 15, 6, 2, Vector2::Zero, 0.1, 0);
		mAnimator->CreateAnimation(L"Transp2", mImage, Vector2((346.33f * 9), (346.33f * 1)), 15, 6, 2, Vector2::Zero, 0.1, 0);
		mAnimator->CreateAnimation(L"Damaged", mImage, Vector2(0.0f, (346.33f * 4)), 15, 6, 1, Vector2::Zero, 0.2, 0);
		mAnimator->CreateAnimation(L"Up", mImage, Vector2((346.33f * 0), (346.33f * 3)), 15, 6, 1, Vector2::Zero, 0.1, 0);
		mAnimator->CreateAnimation(L"Down", mImage, Vector2((346.33f * 11), (346.33f * 3)), 15, 6, 1, Vector2::Zero, 0.1, 0);

		mAnimator->GetCompleteEvent(L"Jump") = std::bind(&Character04::JumpCompleteEvent, this);
		mAnimator->GetCompleteEvent(L"DJump") = std::bind(&Character04::DJumpCompleteEvent, this);
		mAnimator->GetStartEvent(L"Slide") = std::bind(&Character04::SlideStarteEvent, this);
		mAnimator->GetCompleteEvent(L"Transp1") = std::bind(&Character04::Transp1CompleteEvent, this);
		mAnimator->GetCompleteEvent(L"Transp2") = std::bind(&Character04::Transp2CompleteEvent, this);
		mAnimator->GetCompleteEvent(L"Damaged") = std::bind(&Character04::DamagedCompleteEvent, this);
	...
	void Character04::Update()
	{
		...
		if (scn->GetName() == L"Bonus")
		{
			...
			switch (mState)
			{
			case ya::eChar04State::Up:
				up();
				break;
			case ya::eChar04State::Down:
				down();
				break;
			}
		}
		...
		if (scn->GetName() == L"Play" || scn->GetName() == L"Make")
		{
...

			{
				if (mInv == false)// ü�� �ð� ��� ����
				{
					Character04::mCurHp -= (3.0f - ((3.0f) * 0.17)) * Time::DeltaTime();
				}
				else// ���� ������ �Ծ��� ��
				{
			...
			// ���� ü�� ���� ó��
			if (Character04::mCurHp <= 0.0f)
			{
				if (mZom == true)
				{
					mCurHp = 0.0f;
					mState = eChar04State::Death;
			...
			if (Character04::mCurHp > 100.0f)
			{
				mCurHp = 100.0f;
			}

			// ���� ü�� �ۼ�Ʈ ���
			Character04::mHpPercent = (Character04::mCurHp / Character04::mMaxHp) * 100;

			// ���¿� ���� ���� �Լ� �����ϴ� ����ġ��
			switch (mState)
			{
			case ya::eChar04State::Run:
				run();
				break;
			case ya::eChar04State::BeforeRun:
				beforerun();
				break;
			case ya::eChar04State::Jump:
				jump();
				break;
			case ya::eChar04State::DoubleJump:
				djump();
				break;
			case ya::eChar04State::Slide:
				slide();
				break;
			case ya::eChar04State::Death:
				death();
				break;
			case ya::eChar04State::Idle:
				idle();
				break;
			case ya::eChar04State::Transp1:
				transparent();
				break;
			case ya::eChar04State::Transp2:
				mAnimator->Play(L"Transp2", true);
				break;
			case ya::eChar04State::Damaged:
				mAnimator->Play(L"Damaged", true);
				break;
			case ya::eChar04State::BigRun:
				bigrun();
				break;
			}
		}
	...
	void Character04::Render(HDC hdc)
	{
		GameObject::Render(hdc);
	}

	void Character04::Release()
	{
		GameObject::Release();
	}

	void Character04::OnCollisionEnter(Collider* other)
	{
...
#pragma endregion
	...
	void Character04::OnCollisionStay(Collider* other)
	{
	...
	void Character04::OnCollisionExit(Collider* other)
	{
		...
		if (mKeyError == false)
		{
			if (!mRigidbody->GetGround() && Input::GetKeyDown(eKeyCode::W))
			{
				...
				if (mDJmpcnt == 1)
				{
					...
					mRigidbody->SetVelocity(velocity);
					mRigidbody->SetGround(false);
					mState = eChar04State::DoubleJump;
				}
			}
		}
	...
	void Character04::death()
	{
		mAnimator->Play(L"Death", false);
	}

	void Character04::idle()
	{
		mAnimator->Play(L"idle", true);
	}

	void Character04::transparent()
	{
		mAnimator->Play(L"Transp1", true);
	}

	void Character04::bigrun()
	{
		Transform* tr = GetComponent<Transform>();
	...
	void Character04::down()
	{
		...
		if (Input::GetKey(eKeyCode::W))
		{
			...
			//if (mJmpcnt == 1)
			{
				...
				mState = eChar04State::Up;
			}
		}
	}

	void Character04::JumpCompleteEvent(/*const Cuphead* this*/)
	{
		mState = eChar04State::BeforeRun;
	}

	void Character04::DJumpCompleteEvent()
	{
		mState = eChar04State::BeforeRun;
	}

	void Character04::SlideStarteEvent()
	{
		Transform* tr = GetComponent<Transform>();
		tr->SetPos(Vector2(tr->GetPos().x, 650.0f));
	}

	void Character04::Transp1CompleteEvent()
	{
		mState = eChar04State::Damaged;
	}

	void Character04::DamagedCompleteEvent()
	{
		//mState = eChar01State::Transp2;
		mState = eChar04State::Run;
	}

	void Character04::Transp2CompleteEvent()
	{
		mState = eChar04State::Run;
	}
}