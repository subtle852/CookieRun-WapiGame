...
namespace ya
{
	Character01* BonusScene::mCh01 = nullptr;
	bool BonusScene::mCloudEffectOnatPlay = false;
...
#pragma endregion
	...
	void BonusScene::Render(HDC hdc)
	{
		Scene::Render(hdc);
...
#pragma endregion
		...
		int tempp = SelectCharScene::GetCharNumber();

		if (tempp == 1)
		{
			dynamic_cast<Character01*>(PlayScene::mCh01)->mWaitTime = 0.0f;
			dynamic_cast<Character01*>(PlayScene::mCh01)->mPlayInit = false;
		}
		else if (tempp == 2)
		{
			dynamic_cast<Character02*>(PlayScene::mCh01)->mWaitTime = 0.0f;
			dynamic_cast<Character02*>(PlayScene::mCh01)->mPlayInit = false;
		}
		else if (tempp == 3)
		{
			dynamic_cast<Character03*>(PlayScene::mCh01)->mWaitTime = 0.0f;
			dynamic_cast<Character03*>(PlayScene::mCh01)->mPlayInit = false;
		}
		else if (tempp == 4)
		{
			dynamic_cast<Character04*>(PlayScene::mCh01)->mWaitTime = 0.0f;
			dynamic_cast<Character04*>(PlayScene::mCh01)->mPlayInit = false;
		}
		else if (tempp == 5)
		{
			dynamic_cast<Character05*>(PlayScene::mCh01)->mWaitTime = 0.0f;
			dynamic_cast<Character05*>(PlayScene::mCh01)->mPlayInit = false;
		}
		else if (tempp == 6)
		{
			dynamic_cast<Character06*>(PlayScene::mCh01)->mWaitTime = 0.0f;
			dynamic_cast<Character06*>(PlayScene::mCh01)->mPlayInit = false;
		}
		else if (tempp == 7)
		{
			dynamic_cast<Character07*>(PlayScene::mCh01)->mWaitTime = 0.0f;
			dynamic_cast<Character07*>(PlayScene::mCh01)->mPlayInit = false;
		}
		else if (tempp == 8)
		{
			dynamic_cast<Character08*>(PlayScene::mCh01)->mWaitTime = 0.0f;
			dynamic_cast<Character08*>(PlayScene::mCh01)->mPlayInit = false;
		}
	...
	void BonusScene::OnExit()
	{
		...
		//
		//나갈 때 자국 남기고 가니 임시적으로 저 밖으로 생기게 한거임
		Transform* tr = mCh01->GetComponent<Transform>();
		Vector2 pos = tr->GetPos();
		pos.x = -500.0f;
		pos.y = -500.0f;
		...
		object::Destory(mCh01);
		object::Destory(mPet01);
		...
		//GameObject* mOb;
		//std::unordered_map<UINT64, GameObject*> mObs;
