...
namespace ya
{
	float Character05::mCurHp = 100;
	float Character05::mMaxHp = 100;
	float Character05::mHpPercent = (Character05::mCurHp / Character05::mMaxHp) * 100;

	bool Character05::mBig = false;
	bool Character05::mSml = false;
	...
	void Character05::Initialize()
	{
		Transform* tr = GetComponent<Transform>();
		tr->SetScale(Vector2(1.2f, 1.2f));

		Image* mImage = Resources::Load<Image>(L"Char05", L"..\\Resources\\Char\\B\\strawberry.bmp");
		mAnimator = AddComponent<Animator>();
		mAnimator->CreateAnimation(L"idle", mImage, Vector2((322.33f * 2), (322.33f * 2)), 11, 6, 4, Vector2::Zero, 0.13, 0);
		mAnimator->CreateAnimation(L"Roll", mImage, Vector2::Zero, 11, 6, 6, Vector2::Zero, 0.15, 0);
		mAnimator->CreateAnimation(L"BeforeRun", mImage, Vector2((322.33f * 6), (322.33f * 0)), 11, 6, 1, Vector2::Zero, 0.1, 0);
		mAnimator->CreateAnimation(L"Run", mImage, Vector2(0.0f, (322.33f * 1)), 11, 6, 4, Vector2::Zero, 0.1, 0);
		mAnimator->CreateAnimation(L"FastRun", mImage, Vector2((322.33f * 4), (322.33f * 1)), 11, 6, 4, Vector2::Zero, 0.1, 0);
		mAnimator->CreateAnimation(L"SlowRun", mImage, Vector2(0.0f, (322.33f * 1)), 11, 6, 4, Vector2::Zero, 0.5, 0);
		mAnimator->CreateAnimation(L"Jump", mImage, Vector2((322.33f * 0), (322.33f * 0)), 11, 6, 1, Vector2::Zero, 0.1, 0);// 0�� �������� �ִϸ��̼� ���� ����
		mAnimator->CreateAnimation(L"DJump", mImage, Vector2((322.33f * 1), (322.33f * 0)), 11, 6, 6, Vector2::Zero, 0.08, 0);
		mAnimator->CreateAnimation(L"Slide", mImage, Vector2((322.33f * 9), (322.33f * 0)), 11, 6, 1, Vector2::Zero, 0.08, 0);
		mAnimator->CreateAnimation(L"Death", mImage, Vector2((322.33f * 0), (322.33f * 4)), 11, 6, 4, Vector2::Zero, 0.15, 0);
		mAnimator->CreateAnimation(L"Transp1", mImage, Vector2((322.33f * 9), (322.33f * 1)), 11, 6, 2, Vector2::Zero, 0.1, 0);
		mAnimator->CreateAnimation(L"Transp2", mImage, Vector2((322.33f * 9), (322.33f * 1)), 11, 6, 2, Vector2::Zero, 0.1, 0);
		mAnimator->CreateAnimation(L"Damaged", mImage, Vector2(0.0f, (322.33f * 4)), 11, 6, 1, Vector2::Zero, 0.2, 0);
		mAnimator->CreateAnimation(L"Up", mImage, Vector2((322.33f * 0), (322.33f * 3)), 11, 6, 1, Vector2::Zero, 0.1, 0);
		mAnimator->CreateAnimation(L"Down", mImage, Vector2((322.33f * 2), (322.33f * 3)), 11, 6, 1, Vector2::Zero, 0.1, 0);

		mAnimator->GetCompleteEvent(L"Jump") = std::bind(&Character05::JumpCompleteEvent, this);
		mAnimator->GetCompleteEvent(L"DJump") = std::bind(&Character05::DJumpCompleteEvent, this);
		mAnimator->GetStartEvent(L"Slide") = std::bind(&Character05::SlideStarteEvent, this);
		mAnimator->GetCompleteEvent(L"Transp1") = std::bind(&Character05::Transp1CompleteEvent, this);
		mAnimator->GetCompleteEvent(L"Transp2") = std::bind(&Character05::Transp2CompleteEvent, this);
		mAnimator->GetCompleteEvent(L"Damaged") = std::bind(&Character05::DamagedCompleteEvent, this);
	...
	void Character05::Update()
	{
		...
		if (scn->GetName() == L"Bonus")
		{
			...
			switch (mState)
			{
			case ya::eChar05State::Up:
				up();
				break;
			case ya::eChar05State::Down:
				down();
				break;
			}
		}
		...
		if (scn->GetName() == L"Play" || scn->GetName() == L"Make")
		{
...

			{
				if (mInv == false)// ü�� �ð� ��� ����
				{
					Character05::mCurHp -= 3.0f * Time::DeltaTime();
				}
				else// ���� ������ �Ծ��� ��
				{
			...
			// ���� ü�� ���� ó��
			if (Character05::mCurHp <= 0.0f)
			{
				if (mZom == true)
				{
					mCurHp = 0.0f;
					mState = eChar05State::Death;
			...
			if (Character05::mCurHp > mMaxHp)
			{
				mCurHp = mMaxHp;
			}

			// ���� ü�� �ۼ�Ʈ ���
			Character05::mHpPercent = (Character05::mCurHp / Character05::mMaxHp) * 100;

			// ���¿� ���� ���� �Լ� �����ϴ� ����ġ��
			switch (mState)
			{
			case ya::eChar05State::Run:
				run();
				break;
			case ya::eChar05State::BeforeRun:
				beforerun();
				break;
			case ya::eChar05State::Jump:
				jump();
				break;
			case ya::eChar05State::DoubleJump:
				djump();
				break;
			case ya::eChar05State::Slide:
				slide();
				break;
			case ya::eChar05State::Death:
				death();
				break;
			case ya::eChar05State::Idle:
				idle();
				break;
			case ya::eChar05State::Transp1:
				transparent();
				break;
			case ya::eChar05State::Transp2:
				mAnimator->Play(L"Transp2", true);
				break;
			case ya::eChar05State::Damaged:
				mAnimator->Play(L"Damaged", true);
				break;
			case ya::eChar05State::BigRun:
				bigrun();
				break;
			}
		}
	...
	void Character05::Render(HDC hdc)
	{
		GameObject::Render(hdc);
	}

	void Character05::Release()
	{
		GameObject::Release();
	}

	void Character05::OnCollisionEnter(Collider* other)
	{
...
#pragma endregion
	...
	void Character05::OnCollisionStay(Collider* other)
	{
	...
	void Character05::OnCollisionExit(Collider* other)
	{
		...
		if (mKeyError == false)
		{
			if (!mRigidbody->GetGround() && Input::GetKeyDown(eKeyCode::W))
			{
				...
				if (mDJmpcnt == 1)
				{
					...
					mRigidbody->SetVelocity(velocity);
					mRigidbody->SetGround(false);
					mState = eChar05State::DoubleJump;
				}
			}
		}
	...
	void Character05::death()
	{
		mAnimator->Play(L"Death", false);
	}

	void Character05::idle()
	{
		mAnimator->Play(L"idle", true);
	}

	void Character05::transparent()
	{
		mAnimator->Play(L"Transp1", true);
	}

	void Character05::bigrun()
	{
		Transform* tr = GetComponent<Transform>();
	...
	void Character05::down()
	{
		...
		if (Input::GetKey(eKeyCode::W))
		{
			...
			//if (mJmpcnt == 1)
			{
				...
				mState = eChar05State::Up;
			}
		}
	}

	void Character05::JumpCompleteEvent(/*const Cuphead* this*/)
	{
		mState = eChar05State::BeforeRun;
	}

	void Character05::DJumpCompleteEvent()
	{
		mState = eChar05State::BeforeRun;
	}

	void Character05::SlideStarteEvent()
	{
		Transform* tr = GetComponent<Transform>();
		tr->SetPos(Vector2(tr->GetPos().x, 650.0f));
	}

	void Character05::Transp1CompleteEvent()
	{
		mState = eChar05State::Damaged;
	}

	void Character05::DamagedCompleteEvent()
	{
		//mState = eChar01State::Transp2;
		mState = eChar05State::Run;
	}

	void Character05::Transp2CompleteEvent()
	{
		mState = eChar05State::Run;
	}
}